...
import { type GlobalLeaderboardResponse, type LeaderboardEntry } from "../../../models/quiz";
...
const GlobalLeaderboardPage: React.FC = () => {
  ...
  const load = async (p: Period) => {
    ...
    setErr(null);
    try {
      const resp = await quizzesApi.getGlobalTop({ period: p, take: 50 });
  ...
  return (
    <div className="lb-page">
      ...
      <div className="lb-subhead">
        <div className="lb-quiz-name">All quizzes</div>
        <div className="seg">
        ...
        <section className="lb-grid">
          <article className="card board">
            <div className="board-head">
              ...
              <div className="right">
                <span className="muted">Max total: <b>{data.maxScoreTotal}</b></span>
              </div>
            </div>
            ...
            {!!data.top?.length && (
              <ol className="rows">
                {data.top.map(row =>
                  <Row key={`${row.user.id}-${row.rank}`} row={row} highlight={data.myEntry?.user.id === row.user.id} />
                )}
              </ol>
            )}
          </article>
          ...
          {data.myEntry && !myInTop && (
            <article className="card mypos">
              ...
              <div className="muted small">Your current global position (outside top 50).</div>
            </article>
          )}
        </section>
      )}
